...
class LLMOptions:
    ...
    chat_history: dict[str, llm.ChatContext] = field(default_factory=dict)
...
class LLM(llm.LLM):
    ...
    def get_chat_history(self, user_id: str) -> llm.ChatContext:
        """Get chat history for specific user, create if doesn't exist"""
        if user_id not in self._opts.chat_history:
            self._opts.chat_history[user_id] = llm.ChatContext()
        return self._opts.chat_history[user_id]
